...
exports.AddOrders = async(req,res)=>{
    ...
    console.log("my order item",orderItems);
    const deliverydate = moment().add(2,'days').format('YYYY-MM-DD');

    try{
        const createOrder = new orderDb({
            userid:req.userId,address,city,pincode,mobile,state,country,
            orderItems,paymentdetails,itemsPrice,
            shippingPrice,totalPrice,deliveredAt:deliverydate
        })
...
exports.updateOrderStatus = async(req,res)=>{
    ...
    try{
        ...
        if(findOrderDetails.orderstatus == "Processing" && orderStatus == "Confirmed"){
            ...
            res.status(200).json({error:"invalid status"});
        }
    } catch (error){
        res.status(200).json(error);
    }
